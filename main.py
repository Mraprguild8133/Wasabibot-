...
class WasabiStorage:
    ...
    def _initialize_client(self):
        ...
        try:
            ...
            clean_region = WASABI_REGION.replace('s3.', '').replace('.wasabisys.com', '') if WASABI_REGION else 'us-east-1'
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            ...
            await loop.run_in_executor(None, lambda: self.s3_client.head_bucket(Bucket=WASABI_BUCKET))
            return True
        except Exception as e:
    ...
    async def download_file(self, object_key: str, file_path: str, progress_callback=None) -> bool:
        ...
        try:
            ...
            response = await loop.run_in_executor(
                None, 
                lambda: self.s3_client.head_object(Bucket=WASABI_BUCKET, Key=object_key)
            )
...
class FileDatabase:
    ...
    async def initialize(self):
        """Initialize the database by loading data"""
    ...
    async def add_file(self, file_id: str, file_data: Dict[str, Any]):
...
async def list_command(client, message: Message):
    """Handle /list command"""
    files = await file_db.list_files()
...
async def download_command(client, message: Message):
    ...
    file_data = await file_db.get_file(file_id)
...
async def handle_file_upload(client, message: Message):
    ...
    try:
        ...
        if upload_success:
            ...
            await file_db.add_file(file_id, file_metadata)
...
async def handle_callbacks(client, callback_query):
    ...
    if data == "help":
        ...
        # Create a fake message object to pass to download_command
        class FakeMessage:
            def __init__(self, command, reply_text, reply_document):
        ...
        # Create a fake message object to pass to stream_command
        class FakeMessage:
            def __init__(self, command, reply_text):
                self.command = command
                self.reply_text = reply_text
        ...
        # Create a fake message object to pass to web_command
        class FakeMessage:
            def __init__(self, command, reply_text):
                self.command = command
                self.reply_text = reply_text
        ...
        if wasabi_success:
            ...
            await file_db.delete_file(file_id)
